...
function printTable(stationName, forecast) {
  ...
  for (const row of forecast) {
    const dayLabel = row.day !== prevDay ? row.day.padEnd(12) : "".padEnd(12);
    prevDay = row.day;

    const dir = `${row.windDirCompass} (${row.windDirDeg}°)`;
    console.log(
      `| ${dayLabel} | ${row.period.padEnd(6)} | ${String(row.windSpeed).padStart(10)} | ${String(row.gust).padStart(10)} | ${dir.padEnd(14)} | ${String(row.waveHeight).padStart(12)} |`
    );
  }
}
